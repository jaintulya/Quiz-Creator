...
import { X, Copy, Check, Sparkles, Bot } from 'lucide-react';
...
export default function AIPromptModal({ onClose }) {
  ...
  return (
    ...
    >
      <div className="glass-card w-full max-w-xl shadow-2xl border-white/10 animate-slide-up overflow-hidden my-auto">
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b border-white/10 bg-[#1a1714]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#a39e94]">Copy &bull; Paste into ChatGPT or Gemini &bull; Paste JSON back</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Instruction banner */}
        <div className="mx-4 sm:mx-6 mt-4 sm:mt-5 p-3.5 rounded-xl bg-[#241e16] border border-caramel-500/20 flex items-start gap-3">
          <Bot className="w-5 h-5 text-caramel-400 flex-shrink-0 mt-0.5" />
        ...
        <div className="p-4 sm:p-6">
          <div className="relative">
            <pre className="w-full h-44 sm:h-52 overflow-y-auto p-4 rounded-xl bg-[#0d0c0b] border border-white/10 text-[#dedbd3] text-xs leading-relaxed whitespace-pre-wrap font-mono select-all">
              {prompt}
            </pre>
            <button
              onClick={handleCopy}
              className="absolute top-3 right-3 px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 border border-white/15 text-white text-xs font-semibold flex items-center gap-1.5 transition-all shadow-md"
            >
        ...
        <div className="p-4 sm:p-5 border-t border-white/10 bg-[#161412] flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-xs text-[#a39e94] text-center sm:text-left">
            Ready to generate quiz JSON in seconds
          </span>
          <button
            id="copy-prompt-btn"
            onClick={handleCopy}
            className="btn-primary w-full sm:w-auto px-6 py-2.5 text-sm font-bold shadow-caramel-glow"
          >
            {copied ? (
              <>
